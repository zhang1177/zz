<script setup>
import { deleteMqttUserApi, list, register } from '@/api/mqttUser'
import { ElMessage, ElMessageBox } from 'element-plus'
import { onMounted, reactive, ref } from 'vue'

const queryForm = reactive({ username: '', pageNum: 1, pageSize: 10 })
const mqttUserList = ref([]) 
const total = ref(0) 
...
async function fetchData() {
  try {
    ...
    if (res.code === 200 && res.data?.list) {
      mqttUserList.value = res.data.list;
      total.value = res.data.total 
    } else {
      console.error('获取MQTT用户列表失败:', res.message)
    }
  } catch (err) {
...
function addMqttUser() {
  ...
  register(userData).then(response => {
    if (response.code === 200) {
      ElMessage.success('MQTT用户添加成功！');
      fetchData(); 
    } else {
      ElMessage.error('MQTT用户添加失败：' + response.message);
    }
  }).catch(error => {
...
<template>
    ...
    <el-form :model="queryForm" label-width="80px" inline>
      ...
      <el-form-item>
        <el-row gutter="5">
          ...
          <el-col :span="8">
            <el-button type="success" @click="addDialogVisible = true" style="width: 100%;">添加用户</el-button>
          </el-col>
  ...
  <el-card class="knife-card">
    <el-table :data="mqttUserList" border style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="plainPassword" label="密码" />
  ...
  <el-dialog title="添加MQTT用户" v-model="addDialogVisible">
    ...
    <template #footer>
      ...
      <el-button type="primary" @click="addMqttUser">确认</el-button>
    </template>
  </el-dialog>
